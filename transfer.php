...
<div class="container">
  ...
  <div class="card-body">
      ...
      <?php if (isset($_POST['get'])) 
      {
        ...
        $array3 = $con->query("select * from userAccounts where accountNo = '$_POST[otherNo]'");
        {
          ...
          { 
           ...
           $transferredBalance =  $userData['balance'] - 500;
           $row2 = $array3->fetch_assoc();
            echo "<div class='alert alert-success w-50 mx-auto'>
                  <form method='POST'>
                    ...
                    Enter Amount for transfer.
                    <input type='number' name='amount' class='form-control' min='1' max='$transferredBalance' required>
    ...
    if (isset($_POST['transfer']))
    {
      ...
      $array = $con->query("select * from transaction where userId = '$userData[id]' AND action = 'transfer' order by date desc LIMIT 3");
      if ($array ->num_rows > 0) 
      {
         while ($row = $array->fetch_assoc()) 
         {
            ...
            {
              echo "<div class='alert alert-warning bg-info text-white'>Transfer have been made for  tk.$row[debit] from your account at $row[date] in  account no.$row[other]</div>";
            }
